...
On session start, the extension loads the saved state and renders it immediately, then starts the first quota poll in the background so quota refresh does not delay interactive startup.
